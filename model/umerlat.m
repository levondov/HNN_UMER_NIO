...
if 1
    ...
    for i=1:length(fidx)
        ...
        if adddipoles_10deg_flag
            B_l_eff = ao.BEND.FitParams(i,3)*ao.BEND.FitParams(i,4);
            B_l_ps = ao.BEND.FitParams(i,1)*ao.BEND.FitParams(i,2);
            c_10 = a2c(10*pi/180 - THERING{fidx(i)}.BendingAngle,B_l_eff,B_l_ps);
            polyB = B_l_ps*c_10/ridg;
            THERING{fidx(i)}.PolynomB(1) = -polyB;
        end
    end
end
