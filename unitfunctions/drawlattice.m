...
%
% Levon: Need to run buildring_lattice first before running this
%
%
%
...
for i = 1:length(THERING)
    ...
    elseif strcmpi(THERING{i}.FamName,'BPM')
        ...
        NumberOfFinds = NumberOfFinds + 1;
        IconColor = 'k';
        h(length(h)+1) = plot(hAxes, SPos, 0+Offset, 'o', 'Color', IconColor);
        try
        %h(length(h)+1) = text(SPos,1.5,THERING{i}.Name,'Fontsize',13,'Fontweight','bold');
        end
    ...
    if CorrectorFlag && (any(strcmpi(THERING{i}.FamName,{'COR','XCOR','YCOR','HCOR','VCOR','HCM','VCM','HVCM','V corrector'})) || isfield(THERING{i},'KickAngle'))
        % Corrector
        NumberOfFinds = NumberOfFinds + 1;
