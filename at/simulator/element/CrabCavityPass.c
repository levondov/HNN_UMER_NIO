...
{	int c, c6;
	...
	if(le == 0)
		{	
				#pragma omp parallel for default(shared) shared(r_in, num_particles) private(c, c6) 
			for(c = 0;c<num_particles;c++)
			{	c6 = c*6;
			    if(!mxIsNaN(r_in[c6]))
				{
					if(sigphi>0)
						omega_t = k*r_in[c6+5] + phi + sigphi*grandn();
					else
						omega_t = k*r_in[c6+5] + phi;
					if(sigvv>0)
					{	
						nvxa = nvx*(1.0+sigvv*grandn());
						nvya = nvy*(1.0+sigvv*grandn());
					}
					else
					{
						nvxa = nvx;
