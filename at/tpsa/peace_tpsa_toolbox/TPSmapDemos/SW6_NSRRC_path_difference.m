...
% 5T
CurrentOfMainPS = 350;
DataOfID = getIDofNSRRCtps('SW6',CurrentOfMainPS);

Si = 0.0; % The location of SWLS in TLS storage ring.
%Si = 0;
Sf = Si+DataOfID.L;
...
%        end
    ...
    t = HomogeneousTPS(By,0); [n,o,BySc(i)] = GetTPS(t);
    t = HomogeneousTPS(Ax,0); [n,o,AxSc(i)] = GetTPS(t);
    t = HomogeneousTPS(As,0); [n,o,AsSc(i)] = GetTPS(t);
    dAxSc(i) = dAx;
end
...
for i=2:length(Sc)
    ...
    dx0 = X0(i)-X0(i-1);
    dy0 = Y0(i)-Y0(i-1);
    dl0 = dl0+sqrt(ds0^2+dx0^2+dy0^2)-ds0;
end
plot(Sc,X0,'r-o');
dl0
display('Pause > Please press Enter for continue') 
